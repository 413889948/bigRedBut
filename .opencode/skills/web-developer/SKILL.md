...
- `context` - 统一上下文对象（推荐）：`{ projectDir, dataDir, cwd }`
- `projectDir` - 目标代码项目目录（用于在该目录运行测试和执行修复）
- `dataDir` - 数据隔离根目录（用于多项目场景，存放 checkpoint/session/knowledge/fix-log）
